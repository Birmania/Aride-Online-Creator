...
'
'        If MaxMapX <= 20 Then PicScHeight = (MaxMapY + 1) * PIC_Y: PicScWidth = (MaxMapX + 1) * PIC_X
'
...
'        Call WriteINI("INFO", "Maxdreams", Val(MAX_DREAMS), App.Path & "\config.ini")
'        Call WriteINI("INFO", "Maxmapitems", Val(MAX_MAP_ITEMS), App.Path & "\config.ini")
'        Call WriteINI("INFO", "Maxmapx", Val(MaxMapX), App.Path & "\config.ini")
'        Call WriteINI("INFO", "Maxmapy", Val(MaxMapY), App.Path & "\config.ini")
'        Call WriteINI("INFO", "Maxcrafts", Val(MAX_CRAFTS), App.Path & "\config.ini")
'        Call WriteINI("INFO", "Maxmaterials", Val(MAX_MATERIALS), App.Path & "\config.ini")
...
'        Call WriteINI("INFO", "Maxpcraft", Val(MAX_PLAYER_CRAFTS), App.Path & "\config.ini")
...
'        ReDim Dreams(1 To MAX_DREAMS) As DreamRec
...
'        ReDim Crafts(1 To MAX_CRAFTS) As CraftRec
'        For i = 1 To MAX_CRAFTS
'            ReDim Crafts(i).Materials(1 To MAX_MATERIALS) As MaterialRec
'            ReDim Crafts(i).Products(1 To MAX_MATERIALS) As MaterialRec
'        Next i
...
'        y = Val(Parse(2))
'
'        If FileExiste("maps\map" & x & ".aoc") Then
'            ' Check to see if the revisions match
'            If GetMapRevision(x) = y Then
'                ' We do so we dont need the map
'                Call SendData("needsmap" & SEP_CHAR & FileDateTime(App.Path & "\maps\map" & x & ".aoc") & SEP_CHAR & END_CHAR)
...
'            If FileExiste("Maps\map" & z & ".aoc") Then Call DeleteUrlCacheEntry(URL & "map" & z & ".aoc")
'            Call URLDownloadToFile(0, URL & "map" & z & ".aoc", App.Path & "\Maps\map" & z & ".aoc", 0, 0)
'        ElseIf Mid$(URL, Len(URL)) <> "/" And Mid$(rep, 1, 1) = "/" Then
'            If FileExiste("Maps\map" & z & ".aoc") Then Call DeleteUrlCacheEntry(URL & rep & "map" & z & ".aoc")
'            Call URLDownloadToFile(0, URL & rep & "map" & z & ".aoc", App.Path & "\Maps\map" & z & ".aoc", &O10, 0)
'        Else
'            If FileExiste("Maps\map" & z & ".aoc") Then Call DeleteUrlCacheEntry(URL & rep & "map" & z & ".aoc")
'            Call URLDownloadToFile(0, URL & rep & "map" & z & ".aoc", App.Path & "\Maps\map" & z & ".aoc", &O10, 0)
'        End If
...
'            frmmsg.Show
'            Call Envoi(Parse(1), ReadINI("FTP", "NOM", App.Path & "\Config.ini"), ReadINI("FTP", "MDP", App.Path & "\Config.ini"), "Maps\map" & Player(MyIndex).Map & ".aoc", "map" & Player(MyIndex).Map & ".aoc", Parse(2))
...
'    End If
'
'    If LCase$(Parse(0)) = "notwarp" Then If (GetPlayerY(MyIndex) <= 0 Or GetPlayerY(MyIndex) >= MaxMapY Or GetPlayerX(MyIndex) <= 0 Or GetPlayerX(MyIndex) >= MaxMapX) And GettingMap Then GettingMap = False
'
'    ' :::::::::::::::::::::
'    ' :: Map data packet ::
'    ' :::::::::::::::::::::
...
'                    If Player(n).Arrow(x).ArrowY - 1 > MaxMapY Then Player(n).Arrow(x).Arrow = 0: Exit Sub
'                End If
'                If i = DIR_UP Then
'                    Player(n).Arrow(x).ArrowY = GetPlayerY(n) - 1
...
'                    If Player(n).Arrow(x).ArrowX - 1 > MaxMapX Then Player(n).Arrow(x).Arrow = 0: Exit Sub
'                End If
'                If i = DIR_LEFT Then
'                    Player(n).Arrow(x).ArrowX = GetPlayerX(n) - 1
...
Sub SendSaveCraft(ByVal craftNum As Integer)
Dim FileName As String
Dim f  As Long
FileName = App.Path & "\Crafts\Craft" & craftNum & ".aop"
    ...
    Open FileName For Binary As #f
        Put #f, , Crafts(craftNum)
    Close #f
End Sub

Sub SaveArea(ByVal AreaNum As Integer)
Dim FileName As String
Dim f  As Long
...
Sub SendSaveDream(ByVal DreamNum As Integer)
Dim FileName As String
Dim f  As Long
...
FileName = App.Path & "\Pets\Pet" & PetNum & ".fcf"
    ...
    Packet = "SAVEPET" & SEP_CHAR & PetNum & SEP_CHAR & Trim$(Pets(PetNum).nom) & SEP_CHAR & Pets(PetNum).sprite & SEP_CHAR & Pets(PetNum).addForce & SEP_CHAR & Pets(PetNum).addDefence & SEP_CHAR & END_CHAR
    Call SendData(Packet)
    Call EcrireEtat("Sauvegarde des Familliés")
End Sub

Sub SendRequestEditCraft()
    ' TODO if necessary
End Sub

Sub SendRequestEditDream()
    ' TODO if necessary
End Sub
...
Dim I As Long
    ...
    For I = 0 To GetNpcNbDrops(npcNum) - 1
...
Dim f As Long
    ...
    FileName = App.Path & "\skills\skill" & SpellNum & ".aos"
