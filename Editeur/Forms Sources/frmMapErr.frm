...
Private Sub cmdRestaure_Click()
    If curMap > 0 Then
        ...
        If FileExistes(App.Path & "\Maps\map" & curMap & ".aoc") Then Kill App.Path & "\Maps\map" & curMap & ".aoc"
        'Copier le nouveau fichier
        Call FileCopy(App.Path & "\Maps\map" & curMap & "BACKUP.aoc", App.Path & "\Maps\map" & curMap & ".aoc")
        'Fermer la fenêtre
        Call Unload(Me)
    End If
End Sub
...
Dim Dmod As String
    ...
    If FileExistes(App.Path & "\Maps\map" & Map & "BACKUP.aoc") Then
        Dmod = FileDateTime(App.Path & "\Maps\map" & Map & "BACKUP.aoc")
