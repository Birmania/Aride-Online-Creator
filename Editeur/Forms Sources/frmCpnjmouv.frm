...
Dim sprt As Long
Dim sd As Byte
If TempNpcsTab(EditorMouvIndex).id > -1 Then
    x(0).Text = TempNpcsTab(EditorMouvIndex).x(0)
    y(0).Text = TempNpcsTab(EditorMouvIndex).y(0)
End If
'Hasard
If TempNpcsTab(EditorMouvIndex).movementType = 1 Then
    mvh.value = vbChecked
End If
'Imobile
If TempNpcsTab(EditorMouvIndex).movementType = 2 Then
    imob.value = vbChecked
    skin.Enabled = True
Else
     imob.value = vbUnchecked
     skin.Enabled = False
End If
'Boucle
If TempNpcsTab(EditorMouvIndex).movementType = 3 Then
    boucle.value = vbChecked
End If

psh.value = TempNpcsTab(EditorMouvIndex).Hasardp
...
ReDim TempNpcsTab(EditorMouvIndex).x(0 To 2) As Byte
ReDim TempNpcsTab(EditorMouvIndex).y(0 To 2) As Byte
TempNpcsTab(EditorMouvIndex).x(0) = Val(x(0).Text)
TempNpcsTab(EditorMouvIndex).y(0) = Val(y(0).Text)
TempNpcsTab(EditorMouvIndex).x(1) = Val(x(1).Text)
TempNpcsTab(EditorMouvIndex).y(1) = Val(y(1).Text)
TempNpcsTab(EditorMouvIndex).x(2) = Val(x(2).Text)
TempNpcsTab(EditorMouvIndex).y(2) = Val(y(2).Text)
If mvh.value = Checked Then TempNpcsTab(EditorMouvIndex).movementType = 1
If boucle.value = Checked Then TempNpcsTab(EditorMouvIndex).movementType = 3
TempNpcsTab(EditorMouvIndex).Hasardp = Val(psh.value)
If imob.value = Checked Then TempNpcsTab(EditorMouvIndex).movementType = 2
TempNpcsTab(EditorMouvIndex).dir = DIRC
InMouvEditor = False
Unload Me
