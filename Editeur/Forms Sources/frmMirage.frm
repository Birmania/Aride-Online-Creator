...
If HORS_LIGNE = 1 Then
    ...
    Next i
    frmIndex.lstIndex.ListIndex = 0
Else: If GetPlayerAccess(MyIndex) >= ADMIN_DEVELOPER Then Call SendRequestEditCraft Else Call AddText("Vous n'êtes pas autorisé à faire cette action.", BrightRed)
End If
End Sub
...
Else: If GetPlayerAccess(MyIndex) >= ADMIN_DEVELOPER Then Call SendRequestEditDream Else Call AddText("Vous n'êtes pas autorisé à faire cette action.", BrightRed)
End If
End Sub
...
'        descSpeed.Caption = Item(GetPlayerInvItemNum(MyIndex, d + 1)).DexReq & " Dextérité"
...
'        descMS.Caption = "Science: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddSci & " Dexterité: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddDex
