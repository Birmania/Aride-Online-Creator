...
If ReadINI("CONFIG", "Music", ClientConfigurationFile) = 1 Then
...
Public Sub PlaySound(Sound As String)
    If ReadINI("CONFIG", "Sound", ClientConfigurationFile) = 1 Then
        Dim soundPath As String
        
        soundPath = App.Path & "\SFX\" & Sound
    
        If Not FileExist(soundPath) Then Exit Sub
        Call sndPlaySound(soundPath, SND_ASYNC Or SND_NODEFAULT)
    End If
End Sub
...
'Vérifie s'il existe
If Not FileExist(Sound) Then Exit Sub

'Joue la musique
frmMirage.Mediaplayer.URL = App.Path & "\" & Sound
frmMirage.Mediaplayer.settings.setMode "loop", True
End Sub
